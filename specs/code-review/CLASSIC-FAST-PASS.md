...
- A touch-enabled Classic browser can open at 1180px, resize the pane to 390px at 200% CSS zoom, and reach the source and thread drawer without an extra dispatch or provider request. Direct 390px phone entry fails before the add-on opens: the Classic host starts `workspace-collapsed` and its `.workspace-toggle-tab` computes to `display:none`. This is a separate host-entry blocker; resize evidence does not close phone-entry acceptance.
