...
export interface JobConfig extends ResticConfig {
  binary: string; excludes: string[]; migrationAcknowledged: boolean;
  schedule: { enabled: boolean; hours: number[]; minute: number; timezone: string };
}
export function defaultJobConfig(): JobConfig {
  return { ...defaultConfig(), binary:'managed', excludes:['**/node_modules','**/.cache','**/restic/password','**/restic/env.sh'], migrationAcknowledged:false,
...
export function validateJobConfig(value: unknown): JobConfig {
  ...
  const {binary,excludes,migrationAcknowledged,schedule,...base}=value as JobConfig;
  ...
  if(typeof migrationAcknowledged!=='boolean'||!schedule||Object.keys(schedule).some(x=>!['enabled','hours','minute','timezone'].includes(x)))throw Error('Invalid schedule');
  ...
  if(config.enabled&&!migrationAcknowledged)throw Error('Confirm migration and disable previous schedulers before enabling');
  return {...config,binary,excludes:[...excludes],migrationAcknowledged,schedule:{...schedule,hours:[...new Set(schedule.hours)].sort((a,b)=>a-b)}};
}
