...
function RemotePeerSettings() {
  ...
  return html`<div style="padding:.35rem 0;max-width:920px">
    <section style=${C.section}>
      ...
      <div style=${{ ...C.row, marginTop: ".65rem" }}><span style=${C.label}>Fingerprint</span><code style=${C.code}>${identity.fingerprint}</code><button style=${C.danger} onClick=${() => mutate({ action: "rotate_identity", confirmation: globalThis.prompt?.(`Key rotation revokes all current trust. Type ROTATE ${identity.fingerprint} to confirm:`) || "" }, "Identity rotated. Restart Piclaw and re-pair every peer.")} disabled=${busy}>Rotate key</button></div>
