...
test("CR-043/044 explicit reassignment retains history, rejects old lifetime, and leaves prior delivery accepted", async () => {
  ...
  const agent = (target: typeof oldTarget): LocalContext => ({ ...ctx, kind: "agent", actorId: target.incarnation, chatJid: target.chatJid, chatIncarnation: target.incarnation });
  try {
    ...
    await expect(reviewAction(agent(oldTarget), "thread", { threadId: thread.threadId }, store)).rejects.toThrow("unavailable");
    await expect(reviewAction(agent(oldTarget), "reply", { threadId: thread.threadId, body: "late", assignmentEpoch: 1, expectedVersion: 1, requestId: "late-old" }, store)).rejects.toThrow("unavailable");
    expect((await reviewAction(agent(oldTarget), "dispatch", { dispatchId: dispatch.id }, store) as any).items[0]).toMatchObject({ unavailable: true, work_state: "superseded" });
    await expect(reviewAction(agent(nextTarget), "reply", { threadId: thread.threadId, body: "stale epoch", assignmentEpoch: 1, expectedVersion: 2, requestId: "stale-epoch" }, store)).rejects.toThrow("assignment changed");
    const fresh: any = await reviewAction(agent(nextTarget), "thread", { threadId: thread.threadId }, store);
    expect(fresh.assignment_epoch).toBe(2);
    expect(fresh.messages.map((m: any) => m.body)).toEqual(["Keep this API stable"]);
    const reply: any = await reviewAction(agent(nextTarget), "reply", { threadId: thread.threadId, body: "I will keep it stable", assignmentEpoch: 2, expectedVersion: 2, requestId: "new-reply" }, store);
