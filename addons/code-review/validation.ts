...
export function boundedText(
  ...
  if (
    ...
    throw new ReviewError(
      "invalid_input",
      `${name} must be ${allowEmpty ? "" : "non-empty "}text of at most ${max} UTF-8 bytes.`,
    );
  return value;
}
export function validId(value: unknown, name = "id"): string {
  if (typeof value !== "string" || !/^[a-zA-Z0-9:_./@-]{1,256}$/.test(value))
    throw new ReviewError("invalid_input", `Invalid ${name}.`);
  return value;
}
export function identity(value: ReviewIdentity): ReviewIdentity {
  if (!value || !["operator", "agent"].includes(value.kind))
    throw new ReviewError(
      "forbidden",
      "Trusted review identity required.",
      403,
    );
  validId(value.ownerId, "owner");
  validId(value.actorId, "actor");
  if (value.kind === "agent") {
...
export function operator(value: ReviewIdentity): void {
  ...
  if (value.kind !== "operator")
    throw new ReviewError("forbidden", "Operator action required.", 403);
}
export function target(value: LocalTarget): LocalTarget {
  if (!value || typeof value !== "object")
    throw new ReviewError(
      "invalid_input",
      "A verified local target is required.",
    );
  validId(value.chatId, "target chat");
  validId(value.incarnation, "target incarnation");
  boundedText(value.label, "target label", 200);
  if (!value.chatId.startsWith("web:"))
    throw new ReviewError(
      "invalid_input",
      "Only local web chats are supported.",
    );
  return value;
}
...
export function pageSize(limit: unknown, max: number): number {
  ...
  if (!Number.isInteger(limit) || Number(limit) < 1 || Number(limit) > max)
    throw new ReviewError("invalid_input", `Page size must be 1–${max}.`);
  return Number(limit);
}
