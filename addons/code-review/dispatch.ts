...
export interface DispatchInput {
  target: LocalTarget;
  items: Array<{ threadId: string; version: number }>;
  summary?: string;
}
...
export class ReviewService extends ReviewStore {
  private dispatch(who: ReviewIdentity, dispatchId: string): DispatchRow {
    ...
    if (who.kind === "agent") {
      ...
      if (
        ...
        this.missing();
      // A host-authorised agent may start during enqueue before its receipt is persisted.
      // Prepared/rejected intents have not authorised work for this dispatch.
      if (
        !this.database.get(
          "SELECT id FROM attempts WHERE dispatch_id=? AND state IN ('attempting','accepted','unknown')",
          dispatchId,
        )
      )
        this.missing();
  ...
  private eligible(
    ...
    target(input.target);
    boundedText(input.summary ?? "", "summary", LIMITS.commentBytes, true);
    if (
      !Array.isArray(input.items) ||
      !input.items.length ||
      input.items.length > LIMITS.batchItems ||
      new Set(input.items.map((i) => i.threadId)).size !== input.items.length
    )
      throw new ReviewError(
        "invalid_selection",
        "Select 1–50 distinct threads.",
      );
    return input.items.map((item) => {
      const thread = this.thread(who, item.threadId);
      if (thread.review_id !== reviewId) this.missing();
      version(thread.version, item.version);
      if (thread.state !== "open")
        throw new ReviewError(
          "resolved",
          "Only open threads can be sent.",
          409,
        );
      const selected = JSON.parse(thread.target_json) as LocalTarget;
      if (
        selected.chatId !== input.target.chatId ||
        selected.incarnation !== input.target.incarnation
      )
        throw new ReviewError(
          "target_mismatch",
  ...
  preview(who: ReviewIdentity, reviewId: string, input: DispatchInput) {
    ...
    return {
      ...
      items: threads.map((thread) => ({
        ...
        anchor: JSON.parse(thread.anchor_json),
      })),
    };
  }
  submit(
    ...
    return this.mutation(who, m, "submit", { reviewId, input }, () => {
      const threads = this.eligible(who, reviewId, input),
        ...
        time = now();
      this.database.run(
        "INSERT INTO dispatches VALUES(?,?,?,?,?,?,1,?)",
        dispatchId,
        reviewId,
        who.actorId,
        JSON.stringify(input.target),
        input.summary ?? "",
        hashText(stableJson(input)),
        time,
      );
      ...
      this.database.run(
        "INSERT INTO attempts VALUES(?,?,1,'prepared',NULL,NULL,?,?)",
        attemptId,
        dispatchId,
        time,
        time,
  ...
  replyAndSubmit(
    ...
    operator(who);
    const existing = this.thread(who, threadId);
    return this.mutation(
      who,
      m,
      "replyAndSubmit",
      { threadId, body, selected, version: m.expectedVersion, reopen },
      () => {
        const reply = this.reply(
          ...
          body,
          {
            requestId: `${m.requestId}/reply`,
            expectedVersion: m.expectedVersion,
          },
          undefined,
          reopen,
        );
        const receipt = this.submit(
          who,
          existing.review_id,
          { target: selected, items: [{ threadId, version: reply.version }] },
          { requestId: `${m.requestId}/dispatch` },
        );
  ...
  inspectDispatch(who: ReviewIdentity, dispatchId: string) {
    ...
    const items = this.database.all<ItemRow>(
      "SELECT * FROM dispatch_items WHERE dispatch_id=? ORDER BY ordinal",
      dispatchId,
    );
    const visibleItems = items.map((item) => {
      const thread = this.database.get<ThreadRecord>(
    ...
    return {
      ...
      summary:
        who.kind === "operator" ||
        visibleItems.every((item) => !("unavailable" in item))
          ? row.summary
          : "",
      target: JSON.parse(row.target_json),
      items: visibleItems,
      attempts: this.database.all<AttemptRow>(
        "SELECT * FROM attempts WHERE dispatch_id=? ORDER BY number",
        dispatchId,
      ),
    };
  }
  ...
  recoverInterrupted(): number {
    return this.database.run(
      "UPDATE attempts SET state='unknown',error_code='interrupted',updated_at=? WHERE state='attempting'",
      now(),
    ).changes;
  }
  async deliver(who: ReviewIdentity, dispatchId: string, bridge: QueueBridge) {
    ...
    const claimed = this.database.transaction(() => {
      const row = this.dispatch(who, dispatchId),
        attempt = this.database.get<AttemptRow>(
          "SELECT * FROM attempts WHERE dispatch_id=? ORDER BY number DESC LIMIT 1",
          dispatchId,
        )!;
      if (attempt.state !== "prepared") return null;
      const expected = JSON.parse(row.target_json) as LocalTarget;
      const items = this.database.all<ItemRow>(
      ...
      const stale = items.some((item) => {
        ...
        return (
          assigned.chatId !== expected.chatId ||
          assigned.incarnation !== expected.incarnation
        );
      });
      if (stale) {
        this.database.run(
          "UPDATE attempts SET state='rejected',error_code='selection_changed',updated_at=? WHERE id=? AND state='prepared'",
          now(),
          attempt.id,
        );
        this.event(
          ...
          row.review_id,
          "dispatch.rejected",
          { attemptId: attempt.id, errorCode: "selection_changed" },
          null,
          dispatchId,
        );
        return null;
      ...
      const changed = this.database.run(
        "UPDATE attempts SET state='attempting',updated_at=? WHERE id=? AND state='prepared'",
        now(),
        attempt.id,
      ).changes;
    ...
    this.database.transaction(() => {
      this.database.run(
        "UPDATE attempts SET state=?,host_row_id=?,error_code=?,updated_at=? WHERE id=? AND state='attempting'",
        state,
        hostRow,
        errorCode,
        now(),
      ...
      this.event(
        who,
        claimed.row.review_id,
        `dispatch.${state}`,
        { attemptId: claimed.attempt.id, hostRow, errorCode },
        null,
        dispatchId,
      );
    });
  ...
  retry(who: ReviewIdentity, dispatchId: string, m: Mutation) {
    ...
    return this.mutation(who, m, "retry", { dispatchId }, () => {
      ...
      if (attempt.state !== "rejected")
        throw new ReviewError(
          "uncertain_delivery",
          "Only a definitively rejected attempt can be retried.",
          409,
        );
      const items = this.database.all<ItemRow>(
        "SELECT * FROM dispatch_items WHERE dispatch_id=? ORDER BY ordinal",
      ...
      for (const item of items) {
        ...
        if (
          ...
          throw new ReviewError(
            "conflict",
            "Guidance changed; prepare a new explicit send.",
            409,
          );
      }
      const attemptId = id("attempt"),
        time = now();
      this.database.run(
        "INSERT INTO attempts VALUES(?,?,?,'prepared',NULL,NULL,?,?)",
        attemptId,
        dispatchId,
        attempt.number + 1,
        time,
      ...
      this.event(
        ...
        row.review_id,
        "dispatch.retry",
        { attemptId, number: attempt.number + 1 },
        null,
        dispatchId,
      );
      return { dispatchId, attemptId };
  ...
  reconcile(
    ...
    operator(who);
    this.dispatch(who, dispatchId);
    boundedText(input.evidence, "reconciliation evidence", 2048);
    ...
    if (
      input.hostRowId !== undefined &&
      (!Number.isSafeInteger(input.hostRowId) || input.hostRowId < 1)
    )
      throw new ReviewError("invalid_input", "Invalid host receipt.");
    return this.mutation(who, m, "reconcile", { dispatchId, input }, () => {
      ...
      if (attempt.id !== input.attemptId || attempt.state !== "unknown")
        throw new ReviewError(
          "conflict",
          "Only the latest unknown attempt can be reconciled.",
          409,
        );
      this.database.run(
        "UPDATE attempts SET state=?,host_row_id=?,updated_at=? WHERE id=?",
        input.decision,
        input.hostRowId ?? null,
        now(),
        attempt.id,
      );
      this.event(
  ...
  updateWork(
    ...
    if (who.kind !== "agent")
      throw new ReviewError(
        "forbidden",
        "Only an assigned agent reports work.",
        403,
      );
    this.dispatch(who, dispatchId);
    if (
      ...
      throw new ReviewError("invalid_input", "Invalid work state.");
    return this.mutation(
      who,
      m,
      "updateWork",
      { dispatchId, threadId, input },
      () => {
        const row = this.dispatch(who, dispatchId),
          item = this.database.get<ItemRow>(
            "SELECT * FROM dispatch_items WHERE dispatch_id=? AND thread_id=?",
            dispatchId,
            threadId,
          );
        if (!item) return this.missing();
        version(item.version, input.itemVersion);
        const thread = this.database.get<ThreadRecord>(
          "SELECT * FROM threads WHERE id=?",
          threadId,
        )!;
        ...
        const ownResolution =
          ...
          assignmentValid &&
          !!this.database.get(
            "SELECT cursor FROM events WHERE thread_id=? AND kind='thread.resolved' AND actor_id=? AND json_extract(data_json,'$.addressedVersion')=?",
            threadId,
            who.actorId,
            thread.version - 1,
          );
        ...
        if (!stale) {
          version(thread.version, input.threadVersion);
          this.epoch(who, thread, input.assignmentEpoch);
        }
        if (["completed", "superseded", "failed"].includes(item.work_state))
          throw new ReviewError(
            "conflict",
            "Terminal item work cannot be replayed.",
            409,
          );
        this.database.run(
          "UPDATE dispatch_items SET work_state=?,version=version+1 WHERE dispatch_id=? AND thread_id=?",
          input.state,
          dispatchId,
          threadId,
        );
