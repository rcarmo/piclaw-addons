...
test("collector exports bounded Graphite paths without chat or secret data", () => {
  ...
  expect(batch.points.map(point => point.path)).toEqual(expect.arrayContaining([
    "piclaw.compaction.smith_test.local.fast-summary.selective.single_pass.manual.success.none.attempt.count",
    "piclaw.compaction.smith_test.local.fast-summary.selective.single_pass.manual.success.none.duration.ttft_ms",
  ]));
...
test("export spools, sends, and advances its durable checkpoint", async () => {
  ...
  try {
    ...
    expect(received.join("\n")).toContain("duration.ttft_ms 700");
