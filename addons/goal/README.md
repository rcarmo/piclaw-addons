...
- A turn aborted to trigger compaction (Piclaw's mid-turn tool-execution ceiling or context-pressure auto-compaction) is treated as a continuation boundary, not a failure. The assistant turn ends with `stopReason` `aborted`, but the compaction is usually **deferred to the next prompt**, so no `session_compact` event fires during that turn. Because these aborts always follow substantive tool work, a non-errored `aborted` turn that did real tool activity (or that did fire `session_compact`) is treated as a compaction boundary and the loop continues automatically instead of halting until the user runs `/goal resume`. This is the main reason long unattended goals appeared to "stop soon after I leave the web UI" / kept stalling after a turn or a nudge: tool-heavy turns hit the compaction ceiling and the bare `aborted` silently killed the loop. Hard errors (`stopReason` `error` or an `errorMessage`) and user stops with no tool activity still suppress continuation.
