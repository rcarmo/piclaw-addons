...
import { collectUsage, DEFAULT_CONFIG, enqueue, flushSpool, graphiteSegment, loadCheckpoint, saveCheckpoint } from "./usage.js";
...
test("collects local usage into provider/model points and tracks checkpoint", () => {
  const path = fixture(); const batch = collectUsage({ ...DEFAULT_CONFIG, instance_id: "Smith Main" }, loadCheckpoint(path), new Date("2026-08-01T01:00:00Z"), path)!;
  ...
  saveCheckpoint(batch.checkpoint, path); expect(collectUsage(DEFAULT_CONFIG, loadCheckpoint(path), new Date(), path)).toBeNull();
});
test("flushes the durable spool to Carbon", async () => {
  ...
  expect(await flushSpool({ ...DEFAULT_CONFIG, carbon_host: "127.0.0.1", carbon_port: port }, path)).toBe(1);
